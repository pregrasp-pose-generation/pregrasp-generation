...
class Logger:
    ...
    def log(self, energy, E_fc, E_dis, E_pen, E_spen, E_joints, E_tpen, E_function, E_rotation, step, show=False):
        """
        ...
        self.writer.add_scalar('Energy/tpen', E_tpen.mean(), step)
        self.writer.add_scalar('Energy/func', E_function.mean(), step)
        self.writer.add_scalar('Energy/rot', E_rotation.mean(), step)
        ...
        if show:
            print(f'Step %d energy: %f  fc: %f  dis: %f  pen: %f  spen: %f  joints: %f  tpen: %f  func: %f  rot: %f' % (step, energy.mean(), E_fc.mean(), E_dis.mean(), E_pen.mean(), E_spen.mean(), E_joints.mean(), E_tpen.mean(), E_function.mean(), E_rotation.mean()))
