...
class HandModel:
    def __init__(self, mjcf_path, mesh_path, contact_points_path=None, penetration_points_path=None, n_surface_points=0, device='cpu'):
        ...
        self.my_test_point = torch.Tensor([0, 0, 0]).to(device=self.device).view(-1, 1).T
        ...
        self.normalized_direction_vector = None
    ...
    def set_parameters(self, hand_pose, contact_point_indices=None):
        ...
        B = self.global_rotation.size(0)

        standard_vector = torch.tensor([1.0, 0.0, 0.0], device=self.device).repeat(B, 1)
        self.direction_vector = torch.bmm(self.global_rotation, standard_vector.unsqueeze(-1)).squeeze(-1)
        self.normalized_direction_vector = self.direction_vector / torch.norm(self.direction_vector, dim=1, keepdim=True)


        self.current_status = self.chain.forward_kinematics(self.hand_pose[:, 9:])
        if contact_point_indices is not None:
            assert hasattr(self, 'contact_candidates'), 'must pass contact_point_indices.json to constructor'
    ...
    def cal_dis_plane(self, p):
        ...
        for link_name in self.mesh:
            ...
            if 'geom_param' not in self.mesh[link_name]:
                verts = self.current_status[link_name].transform_points(
                    self.mesh[link_name]['vertices'])
                if len(verts.shape) == 2:
                    verts = verts.unsqueeze(0).repeat(p.shape[0], 1, 1)
                verts = torch.bmm(verts, self.global_rotation.transpose(1, 2)) + self.global_translation.unsqueeze(1)
                dis_tmp = (p[:, :3].unsqueeze(1) * verts).sum(2) + p[:, 3].unsqueeze(1)
    ...
    def cal_rotation_pen(self, x_r):
        """
        Calculate rotation pen from hand orientation and hand-obj's orientation
    ...
    def get_surface_points_verts(self):
        """
        Get surface points
        ...
        points = []
        batch_size = self.global_translation.shape[0]
        link_names = ["robot0:ffdistal_child", "robot0:mfdistal_child", "robot0:rfdistal_child", "robot0:lfdistal_child", 'robot0:thdistal_child']
        # link_names = ["robot0:mfmiddle_child", "robot0:mfproximal_child"]
        for link_name in link_names:
            n_surface_points = self.mesh[link_name]['vertices'].shape[0]
            points.append(self.current_status[link_name].transform_points(self.mesh[link_name]['vertices']))
            if 1 < batch_size != points[-1].shape[0]:
                points[-1] = points[-1].expand(batch_size, n_surface_points, 3)
        points = torch.cat(points, dim=-2).to(self.device)
        points = points @ self.global_rotation.transpose(1, 2) + self.global_translation.unsqueeze(1)
    ...
    def get_hand_pose_direction(self):
        """
        Get hand pose's direaction (the link of hand plam & mfproximal_child)

        Returns
        -------
        normalized_vector : (N, 3)
            hand direaction
        """
        # in object coordinate system
        hand_direction_vector_stack=torch.matmul(torch.Tensor([0,0,1]).to(self.device),robust_compute_rotation_matrix_from_ortho6d(self.hand_pose[:, 3:9]).transpose(1, 2))
        return hand_direction_vector_stack
    ...
    def get_plotly_data(self, i, opacity=0.5, color='lightblue', with_contact_points=True, wrist_free=False, pose=None):
        """
        ...
        """
        flag=None
        palm_point = torch.Tensor([0, 0, 0]).to(device=self.device).view(-1, 1).T
        if pose is not None:
            pose = np.array(pose, dtype=np.float32)
        data = []
        for link_name in self.mesh:
            if wrist_free and link_name in ['forearm', 'wrist', 'robot0:forearm', 'robot0:wrist_child']:
                continue
            ...
            if pose is not None:
                ...
                if link_name == "robot0:palm":
                    palm_point=self.current_status[link_name].transform_points(palm_point)
                    palm_point = palm_point @ self.global_rotation[i].T + self.global_translation[i]
                    if (palm_point[:,2]>0):
                        flag=True
                    else:
                        flag=False
                    palm_point = palm_point.detach().cpu()
                    palm_point = palm_point @ pose[:3, :3].T + pose[:3, 3]
            data.append(go.Mesh3d(x=v[:, 0], y=v[:, 1], z=v[:, 2], i=f[:, 0], j=f[:, 1], k=f[:, 2],
                                  color=color, opacity=opacity))
        data.append(go.Scatter3d(x=palm_point[:, 0], y=palm_point[:, 1], z=palm_point[:, 2], mode='markers'))
